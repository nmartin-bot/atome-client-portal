...
import {
  BarChart2, FileText, Edit3, Lock, Zap, Headphones, Settings, LogOut,
...
const NAV: NavItem[] = [
  { type: 'section', label: 'Projet' },
  { type: 'link', to: '/dashboard', icon: BarChart2, label: "Vue d'ensemble" },
  { type: 'link', to: '/dashboard/documents', icon: FileText, label: 'Documents' },
  { type: 'link', to: '/dashboard/brief', icon: Edit3, label: 'Brief métier' },
  { type: 'link', to: '/dashboard/vault', icon: Lock, label: 'Credentials' },
  { type: 'section', label: 'Services' },
  { type: 'link', to: '/dashboard/tokens', icon: Zap, label: 'Tokens & abonnement' },
  { type: 'link', to: '/dashboard/sav', icon: Headphones, label: 'SAV' },
  { type: 'section', label: 'Système' },
  { type: 'link', to: '/dashboard/settings', icon: Settings, label: 'Paramètres' },
]
...
function GlobalSearch() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const q = query.trim()
      const [{ data: docs }, { data: tickets }] = await Promise.all([
        supabase.from('documents').select('id, label').eq('project_id', activeProject.id).ilike('label', `%${q}%`).limit(5),
        supabase.from('sav_tickets').select('id, title').eq('project_id', activeProject.id).ilike('title', `%${q}%`).limit(5),
      ])
      setResults([
        ...(docs ?? []).map(d => ({ id: d.id, type: 'document' as const, label: d.label, sub: 'Document', to: '/dashboard/documents' })),
        ...(tickets ?? []).map(t => ({ id: t.id, type: 'ticket' as const, label: t.title, sub: 'Ticket SAV', to: '/dashboard/sav' })),
      ])
      setLoading(false)
    }, 180)
  ...
  const ICONS = { document: FileText, ticket: Headphones }
  const TYPE_COLORS = {
          ...
          >
            ...
            <div className="flex items-center gap-3 px-4 py-3.5 border-b border-gray-100">
              ...
              <input
                ...
                placeholder="Rechercher un document, un ticket..."
            ...
            <div className="max-h-[420px] overflow-y-auto">
              ...
              {!query && (
                <div className="px-4 py-6 text-center">
                  <p className="text-sm text-gray-400">Tapez pour rechercher dans votre projet</p>
                </div>
              )}
              {results.length > 0 && (
                  ...
                  {results.map((r, i) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="text-xs text-gray-300 shrink-0">{r.sub}</span>
                      </button>
                    )
                  })}
                </div>
    ...
    <aside className="w-60 flex flex-col h-full bg-white border-r border-gray-100 shrink-0">
      ...
      <nav className="flex-1 px-2 py-3 overflow-y-auto space-y-0.5">
        ...
        {!loading && projects.length > 1 && (
          <>
            <p className="px-2 pt-4 pb-1 text-[10px] font-semibold uppercase tracking-widest text-gray-400">Mes projets</p>
            {projects.map(project => (
              <button
                key={project.id}
                onClick={() => setActiveProject(project)}
                className={cn(
                  'w-full flex items-center gap-2.5 px-2.5 py-1.5 rounded-lg text-[13px] text-left transition-colors',
                  activeProject?.id === project.id ? 'bg-gray-100 text-gray-900' : 'text-gray-500 hover:text-gray-900 hover:bg-gray-50'
                )}
              >
                <span
                  className={cn(
                    'w-1.5 h-1.5 rounded-full shrink-0',
                    project.status === 'active' ? 'bg-green-500' : project.status === 'paused' ? 'bg-orange-500' : 'bg-gray-400'
                  )}
                />
      ...
      {/* User profile */}
      <div className="relative px-2 h-16 flex items-center border-t border-gray-100 shrink-0" ref={menuRef}>
        {showUserMenu && (
        ...
        <div className="w-full flex items-center gap-2 px-1 py-1.5 rounded-lg hover:bg-gray-50 cursor-pointer"
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[11px] text-gray-400 leading-tight">Client</p>
          </div>
